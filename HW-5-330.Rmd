...
IM NOT SURE HOW TO TALK ABOUT INTERACTION HERE. Maybe slope?
...
$\beta_1$: Holding all else contstant, as PPGDP goes up by 1, the life expectancy goes up by $\beta_1$, on average.

$\beta_2$: For equal PPGDP levels, a country in group africa has a $\beta_2$ lower life expectancy than a country in group "other", on average. 
...
$$y_i = 50.42 + 2.94(log(PPGDP)_i) - 11.9I(Group_i = africa) + \\ 11.29I(Group_i = oecd) - 0.04(log(PPGDP)_i)(Group_i = afica) - \\ 0.95(log(PPGDP)_i)(Group_i = oecd) + \epsilon_{ip} \text{ where } \epsilon_i \sim \mathcal{N}(\mu,\,\sigma^{2})$$